...
namespace UnrealBuildTool.Rules {
    public class AntilatencyAltEnvironmentSides : ModuleRules {

        public AntilatencyAltEnvironmentSides(ReadOnlyTargetRules Target) : base(Target) {
...
#endif
                var environmentSidesLibraryPath = Path.Combine(binaryDirectory, "AntilatencyAltEnvironmentSides.dll");

#if UE_4_19_OR_LATER
                RuntimeDependencies.Add(environmentSidesLibraryPath);
#else
                RuntimeDependencies.Add(new RuntimeDependency(environmentSidesLibraryPath));
#endif
            } else if (Target.Platform == UnrealTargetPlatform.Android) {
                var pluginPath = Utils.MakePathRelativeTo(ModuleDirectory, Target.RelativeEnginePath);
                var aplPath = Path.Combine(pluginPath, "AntilatencyAltEnvironmentSides_APL.xml");
#if UE_4_19_OR_LATER
