...
namespace UnrealBuildTool.Rules {
    public class AntilatencyPhysicalConfigurableEnvironment : ModuleRules {

        public AntilatencyPhysicalConfigurableEnvironment(ReadOnlyTargetRules Target) : base(Target) {
...
#endif
                var physicalConfigurableEnvironmentLibraryPath = Path.Combine(binaryDirectory, "AntilatencyPhysicalConfigurableEnvironment.dll");

#if UE_4_19_OR_LATER
                RuntimeDependencies.Add(physicalConfigurableEnvironmentLibraryPath);
#else
                RuntimeDependencies.Add(new RuntimeDependency(physicalConfigurableEnvironmentLibraryPath));
#endif
            } else if (Target.Platform == UnrealTargetPlatform.Android) {
                var pluginPath = Utils.MakePathRelativeTo(ModuleDirectory, Target.RelativeEnginePath);
                var aplPath = Path.Combine(pluginPath, "AntilatencyPhysicalConfigurableEnvironment_APL.xml");
#if UE_4_19_OR_LATER
